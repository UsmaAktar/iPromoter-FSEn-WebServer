...
def two_mar_frequency_count(p):
    ...
    for a in DNA:
        for b in DNA:
            combination = a + b
            vector.append(p.count(combination))
...
# Return index of where the frequency of 'c' is a give percentage
# say, p is full string, c='A' and value = 0.2 //20%
# it will return the index were 20% or 'A' already counted
def frequency_index(p,c,value):
    count = p.count(c)
    #print count
    for i in range(len(p)):
        if(p[:i].count(c)>=(count*value)):
            return i

    return i

# Adding feature column selection for ensemble
def add_index(feature,start,end):
